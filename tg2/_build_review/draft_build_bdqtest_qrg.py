...
terms_in_qrg = [ 'skos:prefLabel', 'rdfs:label', 'rdf:about', 'dcterms:description','bdqffdq:hasAuthoritiesDefaults','bdqffdq:hasExpectedResponse','skos:example','bdqffdq:composedOf','skos:note','bdqffdq:Parameter','bdqffdq:Specification','rdf:type','bdqffdq:hasUseCase','bdqffdq:ResourceType' ]
...
with open ("../_review/vocabulary/bdquc_term_versions.csv") as vocabfile: 
	try: 
		vocabDataFrame = pandas.read_csv(vocabfile);
	except pandas.errors.ParserError as e:
		sys.exit("Error reading bdquc vocabulary csv file: {}".format(e)) 
...
with open (inputTermsCsvFilename, newline='') as csvfile:
	...
	try: 
		...
		for useCase in usecaseDict.keys(): 
			...
			definitionRow = vocabDataFrame.loc["bdquc:"+vocabDataFrame["term_localName"]==useCase]
			try: 
