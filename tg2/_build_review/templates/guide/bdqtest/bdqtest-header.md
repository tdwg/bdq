...
Some BDQ Tests also require reference to external data such as standard vocabularies of terms or names (bdq:sourceAuthority), [see Parameterizing the Tests](43-parameterizing-the-tests).
...
1. `Response.result` is the returned result for the Test, i.e., for `Validation` Tests, a strictly controlled vocabulary value ("NOT_ISSUE" or "POTENTIAL_ISSUE" only) for `Issue` Tests, a numeric value or a strictly controlled vocabulary value (consisting of exactly "COMPLETE" or "NOT_COMPLETE" for `Measure` Tests, and a data structure (e.g., a list of key value pairs) for proposed changes for `Amendment` Tests.
