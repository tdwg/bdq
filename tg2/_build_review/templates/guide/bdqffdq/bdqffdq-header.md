...
Below is a fragment in Turtle describing VALIDATION_COUNTRY_FOUND, composed of a `Validation`, linking an `Acted Upon` `Information Element`, a `Criterion`, and the `Resource Type` `Single Record`, with the `Validation` linked to a `Validation Method`, and from there a `Specification`. Also shown is a `Validation Policy` linking this `Validation` to a `Use Case`, but not all of the `bdqffdq:includedInPolicy` properties of that `Validation Policy`.

```turtle
bdqtest:69b2efdc-6269-45a4-aecb-4cb99c2ae134-2025-03-07 a bdqffdq:Validation ;
    rdfs:label "VALIDATION_COUNTRY_FOUND" ;
    dcterms:description "Does the value of dwc:country occur in the bdqval:sourceAuthority?" ;
    dcterms:isVersionOf bdqtest:69b2efdc-6269-45a4-aecb-4cb99c2ae134 ;
    dcterms:issued "2025-03-07"^^xsd:date ;
    dcterms:references <https://doi.org/10.15468/doc-gg7h-s853>,
...
<urn:uuid:04cee4e0-0c83-40cc-8de2-e7391f0a97a9> a bdqffdq:ValidationMethod ;
    ...
    skos:note "Example Implementations Source Code: https://github.com/FilteredPush/geo_ref_qc/blob/v2.0.1/src/main/java/org/filteredpush/qc/georeference/DwCGeoRefDQ.java#L158",
        "Example Implementations: Kurator/FilteredPush geo_ref_qc Library (Morris & Lowery 2025b)",
...
<urn:uuid:0348eccf-fc7f-49b3-88b6-1d911e98cbbb> a bdqffdq:ValidationPolicy ;
...
Formal constraints could be useful for validating RDF data produced by `bdqffdq:Mechanisms`, and for testing conformance of RDF representations to the normative guidance in this standard.  We have not included SHACL shapes for validating RDF representations of BDQ concepts in the BDQ Standard, but an experimental set of SHACL shapes is [available](https://github.com/tdwg/bdq/blob/master/tg2/core/bdqffdq_SHACL_constraints.ttl) along with [python code](https://github.com/tdwg/bdq/tree/master/tg2/_build_review/tools/validate_bdqtest_against_shacl.py) to run it for validating RDF representations of `bdqtest:` Tests and `bdquc:` `Use Cases`.  This validation currently passes, and the python code and SHACL shapes are available to test RDF reports produced by `bdqffdq:Mechanisms` for conformance to the normative guidance in this standard.  
