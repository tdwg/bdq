...
Note that in some cases the `bdqffdq:InformationElement` is replaced with a combination of `Information Elements` which we call an Abstract Information Element Label, e.g. "COORDINATESTERRESTRIALMARINE"

The table below lists the terms used for the Test `Types` and the evaluation terms, with an example for each, along with the few `Abstract 
...
<a  name="abstract_information_elements_label">**Abstract Information Elements Label**</a>
