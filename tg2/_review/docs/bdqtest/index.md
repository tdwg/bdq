...
The terminology of BDQ is based primarily on the Fitness for use Framework (Veiga 2016, Veiga et al. 2017, Biodiversity Information standards (TDWG) Task Group 1) expressed as an ontology, but additional vocabularies are required for a complete description of the Tests and how to use them. See section [2.4 Vocabularies](../index.md#24-vocabularies) in [The Biodiversity Data Quality (BDQ) Standard](../index.md).


### 2.1 Characteristics of the Tests (non-normative)

BDQ Tests evaluate values of a subset of Darwin Core Terms (Darwin Core Maintenance Group 2021), using bdqffdq:InformationElements as inputs. Each Test is designed to be independent, supporting flexible combinations to meet varied data quality needs without enforcing a specific execution model. Tests may be run in parallel, sequentially on individual records, or as queries over datasets or unique values.

Tests are structured in pairs: each Amendment Test is matched with a corresponding Validation Test that assesses a particular aspect of data quality. Amendment Tests may propose improvements to term values; however, the BDQ standard recommends that such improvements be evaluated before application. Some Tests also reference external data sources, such as controlled vocabularies or taxonomic authorities.

Most BDQ Tests are defined for SingleRecords (bdqffdq:SingleRecord), but MultiRecord (bdqffdq:MultiRecord) Tests are also supported—for example, reporting that 75% of records lack a valid dwc:basisOfRecord value. MultiRecord Tests enable the application of Quality Assurance and Quality Control principles by aggregating results across records. Notably, no BDQ Tests use data from other records to assess the quality of a SingleRecord; existing MultiRecord Tests accumulate outputs from other Tests or identify outliers across datasets.

The scope and requirements of each Test are described in its bdqffdq:Specification, which identifies relevant Darwin Core Terms and bdqffdq:InformationElements. Specifications may also cite external authorities (e.g., ISO standards), listed under "Source Authority" (bdq:SourceAuthority) with links to required resources.

### 2.2 Test Types (non-normative)

The concept of 'Test' in the context of BDQ includes four distinct types: Validation (bdqffdq:Validation); Issue (bdqffdq:Issue); Amendment (bdqffdq:Amendment) and Measure (bdqffdq:Measure).
...
We have used Issue Tests for a small number of cases where we wished to flag a value that might indicate a record is not fit for some purpose, but the evaluation of this case would take human review. For example, the Test [ISSUE_ANNOTATION_NOTEMPTY](../docs/terms/bdqtest/index.md#ISSUE_ANNOTATION_NOTEMPTY) is informing the tester than there is at least one annotation associated with record and this should be evaluated before using the record. Similarly for the other two ISSUE-Type Tests: [ISSUE_DATAGENERALIZATIONS_NOTEMPTY](../docs/terms/bdqtest/index.md#ISSUE_DATAGENERALIZATIONS_NOTEMPTY) where some form of transformation has occurred, and the Test [ISSUE_ESTABLISHMENTMEANS_NOTEMPTY](../docs/terms/bdqtest/index.md#ISSUE_ESTABLISHMENTMEANS_NOTEMPTY) where the value needs to be assessed for utility.
...
TDWG Biodiversity Data Quality Interest Group Task Group 2: Data Quality Tests and Assertions. 2025. BDQ Tests and Assertions. Biodiversity Information Standards (TDWG). <http://rs.tdwg.org/../docs/terms/bdqtest/index.md#/2025-04-11>
