...
| parameterizedTest | A test that allows a bdq:Parameter to be set prior to the test being run. Where a bdq:Parameter value has not been provided, a default is specified within the test. | Test |  |
...
| TestPrerequisite | Conditions that must be met for a test to be run (e.g., fields having values, tests that need to be run before the current test, availability of a bdq:sourceAuthority). 
| Test | In BDQ Core, an individual evaluation of bdqffdq:InformationElements for some bdqdim: data quality dimension | BDQ Core | For example, [VALIDATION_COUNTRY_FOUND](https://rs.tdwg.org/bdqcore/terms/69b2efdc-6269-45a4-aecb-4cb99c2ae134) | 
