...
| CONVERTED | A conversion has been proposed to values in the bdqffdq:InformationElements to conform with a targeted geographic reference system. | Term-Action | See Test "AMENDMENT_COORDINATES_CONVERTED" (620749b9-7d9c-4890-97d2-be3d1cde6da8). |
...
| DURATIONINSECONDS | The duration in seconds of bdq:InformationElement dwc:eventDate | Term-Action | See test MEASURE_EVENTDATE_DURATIONINSECONDS |
...
| FROM | An Output bdqffdqInformationElement is being populated from a more primary Input bdqffdq:InformationElement | Term-Action | For example, the Test AMENDMENT_SCIENTIFICNAME_FROM_SCIENTIFICNAMEID (f01fb3f9-2f7e-418b-9f51-adf50f202aea) populates dwc:scienticName from dwc:scientificNameId |
| geodetic coordinate reference system | A coordinate reference system based on a geodetic datum, used to describe positions on the surface of the earth | Test | see Chapman and Wieczorek (2020) |
...
| NOTEMPTY | An bdqffdq:InformationElement contains some non-empty value | Term-Action | For example the Test VALIDATION_EVENTDATE_NOTEMPTY (f51e15a6-a67d-4729-9c28-3766299d2985) |
...
| TestPrerequisite | Conditions that must be met for a test to be run (e.g., fields having values, tests that need to be run before the current test, availability of a bdq:sourceAuthority). | Test | In BDQ Core, an individual evaluation of bdqffdq:InformationElements for some bdqdim: data quality dimension | BDQ Core | For example, VALIDATION_COUNTRY_FOUND (69b2efdc-6269-45a4-aecb-4cb99c2ae134) | 
...
| UNAMBIGUOUS | A combination of bdqffdq:InformationElements is unambiguous in that they align to a unique result given a reference or Source Authority (bdq:sourceAuthority) | Term-Action | For example, the Test VALIDATION_TAXON_UNAMBIGUOUS (4c09f127-737b-4686-82a0-7c8e30841590)	tests if the combination of taxon terms unambiguously resolves to a unque taxon by the Source Authority |
